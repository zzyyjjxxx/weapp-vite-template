...
- `investment`：固定资产投资额，必填，单位为万元。
- `project_hydm`：项目所属国民行业，必填，保存行业编码。
- `keyindustry`：项目所属产业赛道，必填，保存名称。
- `futureindustry`：项目发展方向，必填，保存名称。
- `pred_ys`：项目预计营收，必填，单位为万元。
- `pred_tax`：项目预计税收，必填，单位为万元。
- `pred_rdex`：项目预计研发费用，必填，单位为万元。
- `pred_unitenergy`：项目单位能耗增加值，必填，单位为万元/吨标煤。
- `projectdata`：项目建设内容，必填，使用 `text` 语义，不设置字符数限制。
...
- `is_financing`：是否有融资需求，必填；新建表单默认“没有”，读取到空值或缺失值时也归一化为“没有”。
